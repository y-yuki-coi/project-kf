...
bi = [100 0 1000 100] ; % b1 b2 bk bg
ki = [10000 10000 10000] ; % k1 k2 kg
yg = 0.00; % terrain (horizontal)
u0 = 6 ; % constrant neural input 4-8(slow walk--fast run)
Gi = 1000 ; % Feedback gain
% Initial condition
l1 = li(2); l1 = li(2);
...
iter = 100 ; 
Y = [Y;zeros(iter,12)]; Fgi=zeros(iter,3); dt2_xi = zeros(iter,6);
for t = 1:iter
...
skip = 1 ;
nn = 1 ; % mov index
v = VideoWriter('Actuator_Mobility_160612.mp4','MPEG-4');
open(v)
